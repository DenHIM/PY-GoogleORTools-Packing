...
class BinPackingCost:
    data = {}

    def __init__(self, weights, bin_capacities=[100], bin_costs=[1]):
        if len(bin_capacities) != len(bin_costs):
            raise ValueError("bin_capacities and bin_costs must have the same length.")
        
        self.init_data_model(weights, bin_capacities, bin_costs)

    def init_data_model(self, weights, bin_capacities, bin_costs):
        """Create the data for the example."""
        self.data["weights"] = weights
        self.data["items"] = list(range(len(weights)))
        self.data["bins"] = []
        for j, bin_cap in enumerate(bin_capacities):
            self.data["bins"] += [(bin_cap, bin_costs[j]) for _ in self.data["items"]]
    ...
    def run_solve(self, show_IDs=True):
        ...
        # Objective: minimize the number of bins used.
        solver.Minimize(solver.Sum([y[j] * bin_cap[1] for j, bin_cap in enumerate(self.data["bins"])]))
